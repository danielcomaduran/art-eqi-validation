...
class ART:
    ...
    def single_channel_artifact_removal(self, eeg_data) -> np.ndarray:
        ...
        eeg_components = self.compute_kmeans(eeg_embedded, eeg_features)
        ...
        eeg_mask = np.sum(
            eeg_components[fd_mask, :], 0
        )  # Vector with artifact points != 0
    ...
    def compute_features(self, eeg_embedded) -> np.ndarray:
        ...
        f2 = np.sqrt((np.diff(eeg_embedded, axis=0)).var(axis=0)) / eeg_embedded.var(
            axis=0
        )  # H_mobility
    ...
    def compute_fd_mask(self, eeg_components):
        ...
        x_norm = np.repeat(
            np.reshape(np.linspace(0, 1, self.n), [-1, 1]), self.n_clusters, axis=1
        )
        ...
        y_den = matlib.repmat(
            np.amax(eeg_components, axis=1, keepdims=True)
    ...
    def mean_antidiag(self, input_mat, method):
        ...
        if method == "simple":
            ...
            for i_diag, k_diag in enumerate(range(-input_shape[0] + 1, input_shape[1])):
                average_vect[-i_diag - 1] = input_flip.diagonal(
                    offset=k_diag
                ).mean()  # Organize values from end to start to get right order

        elif method == "mask":
            max_diag = (input_flip.diagonal(offset=0)).size  # Size of longest diagonal
            diag_mat = np.zeros(
                (max_diag, self.n)
            )  # Empty matrix to store antidiagonal values
            ...
            for i, k_diag in enumerate(range(-input_shape[0] + 1, input_shape[1])):
                diag_vals = input_flip.diagonal(
                    offset=k_diag
                )  # Values of the k^th diagonal
